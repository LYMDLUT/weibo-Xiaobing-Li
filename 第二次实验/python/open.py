    ...
    def openimage(self):
        ...
        for i in range(len(t)):
            if self.comboBox_2.currentText() == '平面波对理想导体平面边界的垂直入射':
                ...
                # 磁场
                H1 = (-1)*2 * Ei0 / eta1 * np.cos(k1 * z) * np.cos(omiga * t[i]) * scale  # 磁场公式
                # 绘制
            ...
            if self.comboBox_2.currentText() == '平面波对理想介质的垂直入射':
                ...
                E = np.append(E1, E2)
                # 介质1、2磁场
                H1 = (-1)*Ei0 / eta1 * (np.cos(omiga * t[i] - k1 * z1) - R * np.cos(omiga * t[i] + k1 * z1))  # 磁场公式
                H2 = (-1)*T * Ei0 / eta2 * np.cos(omiga * t[i] - k2 * z2)
                # scale = 100
